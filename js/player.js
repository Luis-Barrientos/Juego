...
export function playerUpdate(p, dt, hooks) {
  const { onStairs, onChest, onEnemyHit, onAltar, onCircle, onTome, onKeyPedestal } = hooks;
  ...
  if (input.keys['KeyE']) {
    ...
    // Library Sala de la Llave (rune-pair pedestals).
    if (input.keys['KeyE'] && onKeyPedestal && onKeyPedestal()) {
      input.keys['KeyE'] = false;
    }
  }
}
